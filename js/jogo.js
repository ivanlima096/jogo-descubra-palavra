...
window.addEventListener("load", () => {
  if (!localStorage.getItem("jogoPalavra")) {
    alert(`Cadastre alguma palavra antes de jogar`)
...
const verificarFim = () => {
  ...
  if (chances == 0) {
    respMensagemFinal.className = "display-5 text-danger"
    respMensagemFinal.innerText = `Ah... a palavra era ${palavraSorteada}. Boa sorte na próxima!`
    concluirJogo()
  } else if (respPalavra.innerText == palavraSorteada) {
    respMensagemFinal.className = "display-5 text-success"
    respMensagemFinal.innerText = `Você acertou a palavra: ${palavraSorteada}. Parabéns!!`
    trocarStatus(4)
    concluirJogo()
  }
}
